...
| REPO-01 | Single canonical Web OSINT repo | Partial | Runtime/deployment can be launched from `web-osint-platform` without relying on a separate editable live source tree. **Progress 2026-07-05**: live `docker-compose.yml` synced to canonical (bare service names); live `.env` populated with all 22 canonical-required vars (in-network URLs + ports + bind host); normalizer recreated against canonical compose and verified consuming the `evidence.capture.events.v1` backlog (Rebrowser captures now materialize in the Source Library). Remaining: recreate the other stale workers (dashboard, connect-shadow, research-planner) against the canonical compose, then retire the legacy live tree per REPO-04. | Recreate remaining stale workers against the canonical compose; then proceed to REPO-04 legacy-tree retirement. |
